  ...
  async execute(request: DelegateSupervisorRequest): Promise<void> {
    ...
    if (!fromSupervisor) {
      throw new NotFoundException({
        details: [
          {
            field: 'fromSupervisorUserId',
            message: `Supervisor with ID ${fromSupervisorUserId} not found`,
          },
        ],
      });
    }
    ...
    if (!toSupervisor) {
      throw new NotFoundException({
        details: [
          {
            field: 'toSupervisorUserId',
            message: `Supervisor with ID ${toSupervisorUserId} not found`,
          },
        ],
      });
    }
    ...
    if (fromSupervisorUserId === toSupervisorUserId) {
      throw new BadRequestException({
        details: [
          {
            field: 'toSupervisorUserId',
            message: 'Cannot delegate responsibilities to the same supervisor',
          },
        ],
      });
    }
