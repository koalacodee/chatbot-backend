...
export class UpdateMainDepartmentUseCase {
  ...
  async execute(id: string, input: UpdateMainDepartmentInput, userId?: string): Promise<Department> {
    ...
    if (userId) {
      ...
      if (userRole === Roles.SUPERVISOR) {
        ...
        if (!supervisorDepartmentIds.includes(id)) {
          throw new ForbiddenException('You do not have access to update this department');
        }
      }
